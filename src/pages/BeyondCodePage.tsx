...
const BeyondCodePage: React.FC = () => (
  <Section title="Beyond Code">
    <div className="text-pink-100">
      <p className="text-center mb-10 max-w-2xl mx-auto whitespace-pre-line">{pageIntro}</p>
      <div className="flex flex-col gap-8">
        {beyondCodeData.map((section) => (
          <div key={section.category}>
            ...
            {section.description && (
              <p className="text-center mb-4 text-base">{section.description}</p>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-4 sm:gap-y-6 lg:gap-y-8 justify-items-center">
              {section.items.map((item) => (
                <div key={item.title} className="bg-[#2d006e]/80 rounded-2xl shadow p-4 border border-pink-200 flex flex-col items-center max-w-xs w-full min-w-[200px]">
